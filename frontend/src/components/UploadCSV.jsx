  ...
  return (
    <div
      ...
      style={{
        ...
        background: dragging ? `rgba(0,229,255,0.04)` : uploaded ? `rgba(0,229,255,0.03)` : "var(--surface)",
        position: "relative",
        overflow: "hidden",
      }}
    >
      {/* Glow strip on top when uploaded */}
      {uploaded && (
        <div style={{
      ...
      <div style={{ fontSize: 10, letterSpacing: 5, color: color, textTransform: "uppercase", marginBottom: 10, fontFamily: "var(--font-display)" }}>
        {label}
      </div>
      ...
      {loading ? (
        ...
        <div>
          ...
          <div style={{ fontSize: 13, color: color, fontWeight: 500 }}>{fileName}</div>
          <div style={{ fontSize: 11, color: "var(--muted2)", marginTop: 4 }}>Click to replace</div>
        </div>
      ) : (
        <div>
          <div style={{ fontSize: 26, marginBottom: 8 }}>📂</div>
          <div style={{ fontSize: 13, color: "var(--text)", fontWeight: 500 }}>Drop {label}.csv here</div>
